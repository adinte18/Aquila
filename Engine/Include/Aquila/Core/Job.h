...
enum class JobPriority : uint8 { Low = 0, Normal = 1, High = 2, Critical = 3 };
...
class JobSystem {
  public:
	...
	auto Schedule(JobPriority priority, const std::string &debugName, Func &&func, Args &&...args)
	...
	auto ScheduleNormal(const std::string &debugName, Func &&func, Args &&...args) {
		return Schedule(JobPriority::Normal, debugName, std::forward<Func>(func), std::forward<Args>(args)...);
	}

	template <typename Func, typename... Args>
	auto ScheduleHigh(const std::string &debugName, Func &&func, Args &&...args) {
		return Schedule(JobPriority::High, debugName, std::forward<Func>(func), std::forward<Args>(args)...);
	}
