...
bool MediaCondition::Evaluate(float w, float h) const {
	...
	switch (op) {
	case Op::Less:      return v <  value;
	case Op::LessEq:    return v <= value;
	case Op::Greater:   return v >  value;
	case Op::GreaterEq: return v >= value;
	case Op::Equal:     return std::abs(v - value) < 0.5f;
	}
	return false;
}
...
static bool EvaluateBlock(const MediaBlock &block, float w, float h) {
	for (const auto &cond : block.conditions) {
		if (!cond.Evaluate(w, h)) return false;
	}
	return true;
}
...
void StyleSheet::AddRule(StyleRule::SelectorType type, std::string selector,
                         std::string pseudoClass, StyleProperties props) {
	int32 specificity = 0;
	switch (type) {
	case StyleRule::SelectorType::Type:  specificity = 1;   break;
	case StyleRule::SelectorType::Class: specificity = 10;  break;
	case StyleRule::SelectorType::Id:    specificity = 100; break;
	}
...
void StyleSheet::ApplyMatchingRules(ComputedStyle &out, const std::vector<StyleRule> &rules,
	...
	for (const auto &rule : rules) {
		if (Matches(rule, view)) matching.push_back(&rule);
	}
...
ComputedStyle StyleSheet::Resolve(const Core::View &view, const ComputedStyle *parentComputed,
	...
	// Base CSS rules, sorted by specificity (lowest first so highest wins last).
	std::vector<const StyleRule *> matching;
	...
	// @media rules — evaluated against the canvas viewport size.
	...
	// @container rules — evaluated against the direct parent element size.
...
bool StyleSheet::Matches(const StyleRule &rule, const Core::View &view) const {
	...
	if (rule.pseudoClass.empty())          return true;
	if (rule.pseudoClass == "hover")       return view.IsHovered();
	if (rule.pseudoClass == "pressed")     return view.IsPressed();
	if (rule.pseudoClass == "focus")       return view.IsFocused();
	if (rule.pseudoClass == "disabled")    return !view.IsEnabled();
...
void StyleSheet::ApplyProperties(ComputedStyle &out, const StyleProperties &props) const {
	...
	if (props.borderColor)      out.borderColor       = *props.borderColor;
	if (props.borderWidth)      out.borderWidth       = *props.borderWidth;
	if (props.borderRadius)     out.borderRadius      = *props.borderRadius;
	if (props.opacity)          out.opacity           = *props.opacity;
	if (props.overflow)         out.overflow          = *props.overflow;
	if (props.display)          out.display           = *props.display;
	if (props.width)            out.width             = *props.width;
	if (props.height)           out.height            = *props.height;

	if (props.min)        { out.minWidth = *props.min; out.minHeight = *props.min; }
	if (props.max)        { out.maxWidth = *props.max; out.maxHeight = *props.max; }
	if (props.minWidth)   out.minWidth   = *props.minWidth;
	if (props.maxWidth)   out.maxWidth   = *props.maxWidth;
	if (props.minHeight)  out.minHeight  = *props.minHeight;
	if (props.maxHeight)  out.maxHeight  = *props.maxHeight;

	if (props.padding)        out.padding        = *props.padding;
	if (props.paddingLeft)    out.padding.left   = *props.paddingLeft;
	if (props.paddingRight)   out.padding.right  = *props.paddingRight;
	if (props.paddingTop)     out.padding.top    = *props.paddingTop;
	if (props.paddingBottom)  out.padding.bottom = *props.paddingBottom;
	if (props.gap)            out.gap            = *props.gap;

	if (props.flexDirection)  out.flexDirection = *props.flexDirection;
	if (props.justifyContent) out.justify       = *props.justifyContent;
	if (props.alignItems)     out.align         = *props.alignItems;
	if (props.flexWrap)       out.wrap          = *props.flexWrap;
	if (props.flexGrow)       out.flexGrow      = *props.flexGrow;

	if (props.position)  out.position = *props.position;
	if (props.top)       out.top      = *props.top;
	if (props.right)     out.right    = *props.right;
	if (props.bottom)    out.bottom   = *props.bottom;
	if (props.left)      out.left     = *props.left;
	if (props.zIndex)    out.zIndex   = *props.zIndex;

	if (props.color)              out.color              = *props.color;
	if (props.accentColor)        out.accentColor        = *props.accentColor;
	if (props.fontSize)           out.fontSize           = *props.fontSize;
	if (props.fontFamily)         out.fontFamily         = *props.fontFamily;
	if (props.transitionDuration) out.transitionDuration = *props.transitionDuration;
	if (props.transitionEasing)   out.transitionEasing   = *props.transitionEasing;
	if (props.textAlign)          out.textAlign          = *props.textAlign;
	if (props.boxShadows)         out.boxShadows         = *props.boxShadows;
}
