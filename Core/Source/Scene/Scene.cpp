 ...
 * the scene.
 */
EntityManager *AquilaScene::GetEntityManager() { return m_EntityManager.get(); }

/**
 * @brief Retrieves the SceneGraph associated with the scene.
 *
 * @return SceneGraph* Pointer to the SceneGraph that manages the hierarchy and
 * relationships of entities in the scene.
 */
SceneGraph *AquilaScene::GetSceneGraph() { return m_SceneGraph.get(); }
 ...
 */
const std::string &AquilaScene::GetSceneName() { return m_SceneName; }

/**
 * @brief Initializes the scene by creating an EntityManager and a SceneGraph.
 *
 * This function is called when the scene starts, setting up the necessary
 ...
 *
 * This function is called when the scene is no longer needed, ensuring that all
 * resources associated with the scene are properly released.
 */
  ...
  for (auto &[idStr, entityData] : entitiesJson.items()) {
    ...
    if (entityData.contains("SceneNodeComponent")) {
      ...
      if (parentUUID == "null") {
        ...
        node.Parent = uuidToEntity.count(parentUUID)
                          ? uuidToEntity.at(parentUUID)
                          : entt::null;
      }
 ...
 *
 * This function writes the current state of the scene, including entities and
 * their components, to a JSON file specified by the filepath.
 *
  ...
  for (auto &entity : view) {
    ...
    if (registry.all_of<TransformComponent>(entity)) {
      ...
      entityJson["TransformComponent"] = {
          ...
          {"Scale",
           {transform.GetLocalScale().x, transform.GetLocalScale().y,
    ...
    if (registry.all_of<SceneNodeComponent>(entity)) {
      ...
      entityJson["SceneNodeComponent"] = {
          {"Parent",
           node.Parent == entt::null
               ? "null"
               : registry.get<MetadataComponent>(node.Parent).ID.ToString()},
          {"Children", nlohmann::ordered_json::array()}};

      for (auto &child : node.Children) {
        entityJson["SceneNodeComponent"]["Children"].push_back(
            registry.get<MetadataComponent>(child).ID.ToString());
      }
    }
    ...
    if (registry.all_of<LightComponent>(entity)) {
      ...
      entityJson["LightComponent"] = {
          {"Type",
           static_cast<int>(
               light.m_Type)}, // store as int: 0=Point,1=Directional,2=Spot
    ...
    sceneJson["Entities"][std::to_string(static_cast<int>(entity))] =
        entityJson;
  }
