 ...
 *
 * This function returns a pointer to the active scene managed by the
 * SceneManager. If no scene is active, it will return a nullptr.
 *
 * @return AquilaScene* Pointer to the active scene, or nullptr if no scene is
 * active.
 */
AquilaScene *SceneManager::GetActiveScene() const { return m_ActiveScene; }

/**
 * @brief Changes the active scene to the one specified by name.
 *
 * If the scene is found, it will be set as the active scene and its OnStart
 * method will be called. If the scene is not found, an exception will be
 * thrown. @todo : maybe i should not throw an exception here?
 *
 ...
 */
bool SceneManager::HasScene() const { return !m_Scenes.empty(); }
 ...
 *
 * This function changes the active scene to the pending scene and clears the
 * pending change. If an activation callback is set, it will be called with the
 * newly activated scene.
 */
