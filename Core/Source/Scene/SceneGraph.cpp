 ...
 * registry. It initializes the SceneNodeComponent and establishes the
 * parent-child relationship if a parent is specified.
 *
 * @param registry The entt registry containing the scene graph.
 ...
 */
void SceneGraph::AddChild(entt::registry &registry, entt::entity parent,
                          entt::entity child) {
 ...
 */
void SceneGraph::AttachTo(entt::registry &registry, entt::entity parent,
                          entt::entity node) {
  if (!registry.valid(parent) || !registry.valid(node))
    return;
 ...
 * otherwise.
 */
bool SceneGraph::IsDescendant(entt::registry &registry,
                              entt::entity potentialParent,
  ...
  for (const auto &child : node->Children) {
    if (child == entityToCheck)
      return true;
    if (IsDescendant(registry, child, entityToCheck))
      return true;
  }
  return false;
}
 ...
 */
void SceneGraph::RemoveChild(entt::registry &registry, entt::entity parent,
                             entt::entity child) {
  if (!registry.valid(parent) || !registry.valid(child))
    return;
 ...
 */
void SceneGraph::RemoveAllChildren(entt::registry &registry,
                                   entt::entity parent) {
