...
namespace Engine {
    SynchronizationManager::SynchronizationManager(Device& device, uint32_t framesInFlight)
    ...
    VkSemaphore SynchronizationManager::GetSemaphore(const std::string& name, uint32_t frameIndex) {
    ...
    void SynchronizationManager::CreateFence(const std::string& name, bool startSignaled) {
        ...
        for (uint32_t i = 0; i < m_FramesInFlight; ++i) {
            if (vkCreateFence(m_Device.GetDevice(), &fenceInfo, nullptr, &fenceSet.fences[i]) != VK_SUCCESS) {
                throw std::runtime_error("Failed to create fence: " + name + "_" + std::to_string(i));
            }
        }
    ...
    VkFence SynchronizationManager::GetFence(const std::string& name, uint32_t frameIndex) {
    ...
    bool SynchronizationManager::IsFenceSignaled(const std::string& name, uint32_t frameIndex) {
        VkFence fence = GetFence(name, frameIndex);
        return vkGetFenceStatus(m_Device.GetDevice(), fence) == VK_SUCCESS;
    }

    void SynchronizationManager::WaitForFence(const std::string& name, uint32_t frameIndex) {
        VkFence fence = GetFence(name, frameIndex);
        vkWaitForFences(m_Device.GetDevice(), 1, &fence, VK_TRUE, UINT64_MAX);
    }

    void SynchronizationManager::ResetFence(const std::string& name, uint32_t frameIndex) {
        VkFence fence = GetFence(name, frameIndex);
