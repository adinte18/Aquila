...
CommandBuffer::CommandBuffer(Device &device, VkCommandPool commandPool,
                             CommandBufferType type, const std::string name)
...
void CommandBuffer::Begin(VkCommandBufferUsageFlags flags) {
  if (m_IsRecording) {
    throw std::runtime_error("Command buffer " + m_Name +
                             " is in usage already");
  }
