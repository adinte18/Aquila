...
Buffer::Buffer(Device &device, const std::string &debugName,
               VkDeviceSize instanceSize, uint32_t instanceCount,
               VkBufferUsageFlags usageFlags,
               VkMemoryPropertyFlags memoryPropertyFlags,
               VkDeviceSize minOffsetAlignment)
    : m_Device{device}, m_InstanceSize{instanceSize},
  ...
  device.CreateBuffer(m_BufferSize, usageFlags, memoryPropertyFlags, m_Buffer,
                      m_Memory);
  device.SetObjectDebugName(VK_OBJECT_TYPE_BUFFER,
                            reinterpret_cast<uint64>(m_Buffer),
                            debugName.c_str());
}
