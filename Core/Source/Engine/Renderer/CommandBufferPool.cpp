...
CommandBufferPool::CreateCommandBuffer(CommandBufferType type,
                                       const std::string &name) {
  return CreateUnique<CommandBuffer>(m_Device, m_Pool, type, name);
}

std::vector<Unique<CommandBuffer>>
CommandBufferPool::CreateCommandBuffers(CommandBufferType type, uint32_t count,
  ...
  for (uint32_t i = 0; i < count; ++i) {
    std::string name =
        baseName.empty() ? "" : baseName + "_" + std::to_string(i);
