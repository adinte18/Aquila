...
bool Device::CheckDeviceExtensionSupport(VkPhysicalDevice vkPhysicalDevice) {
  ...
  vkEnumerateDeviceExtensionProperties(vkPhysicalDevice, nullptr,
                                       &extensionCount, nullptr);

  std::vector<VkExtensionProperties> availableExtensions(extensionCount);
  vkEnumerateDeviceExtensionProperties(
      vkPhysicalDevice, nullptr, &extensionCount, availableExtensions.data());
...
Device::FindQueueFamilies(VkPhysicalDevice vkPhysicalDevice) {
  ...
  for (const auto &queueFamily : queueFamilies) {
    ...
    vkGetPhysicalDeviceSurfaceSupportKHR(vkPhysicalDevice, i, m_Surface,
                                         &presentSupport);
...
void Device::DestroyDebugMessengerEXT(VkInstance instance,
...
Device::QuerySwapChainSupport(VkPhysicalDevice vkPhysicalDevice) {
  VkSwapChainSupportDetails details;

  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(vkPhysicalDevice, m_Surface,
                                            &details.m_SurfaceCapabilities);

  uint32_t formatCount;
  vkGetPhysicalDeviceSurfaceFormatsKHR(vkPhysicalDevice, m_Surface,
                                       &formatCount, nullptr);

  if (formatCount != 0) {
    details.m_Formats.resize(formatCount);
    vkGetPhysicalDeviceSurfaceFormatsKHR(
        vkPhysicalDevice, m_Surface, &formatCount, details.m_Formats.data());
  }

  uint32_t presentModeCount;
  vkGetPhysicalDeviceSurfacePresentModesKHR(vkPhysicalDevice, m_Surface,
...
void Device::SetObjectDebugName(VkObjectType objectType, uint64 handle,
                                const char *name) {
  if (!enableValidationLayers)
    return;
  ...
  auto func = (PFN_vkSetDebugUtilsObjectNameEXT)vkGetInstanceProcAddr(
      m_VulkanInstance, "vkSetDebugUtilsObjectNameEXT");
  if (func != nullptr) {
