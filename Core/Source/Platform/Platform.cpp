...
// Initialize platform info
  ...
  s_PlatformInfo.cpuCores =
      static_cast<int>(std::thread::hardware_concurrency());

  struct sysinfo info;
  if (sysinfo(&info) == 0) {
    s_PlatformInfo.totalMemory =
        static_cast<std::size_t>(info.totalram * info.mem_unit);
  }
...
const PlatformSpec &GetPlatformInfo() { return s_PlatformInfo; }
