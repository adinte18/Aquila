...
ThreadHandle *CreateThread(int (*function)(void *), const char *name,
                           void *data) {
  ThreadHandle *handle = new ThreadHandle();
  handle->sdlThread = SDL_CreateThread(function, name, data);
  if (!handle->sdlThread) {
    Aquila::LogError("Failed to create thread: " + std::string(SDL_GetError()));
    delete handle;
    return nullptr;
  }
  return handle;
...
#ifdef AQUILA_DEBUG
    Aquila::Log("Thread " + std::string(SDL_GetThreadName(thread->sdlThread)) +
                " finished with status: " + std::to_string(*status));
#endif
    delete thread;
  }
}
...
MutexHandle *CreateMutex() {
  ...
  if (!handle->sdlMutex) {
    Aquila::LogError("Failed to create mutex: " + std::string(SDL_GetError()));
    delete handle;
    return nullptr;
  }
  return handle;
...
std::uint32_t GetCurrentThreadId() { return SDL_ThreadID(); }

std::uint32_t GetHardwareThreadCount() {
  return static_cast<std::uint32_t>(SDL_GetNumLogicalCPUCores());
}

void Sleep(std::uint32_t milliseconds) { SDL_Delay(milliseconds); }
