...
PrefilterRenderingSystem::PrefilterRenderingSystem(Engine::Device &device,
                                                   VkRenderPass renderPass,
                                                   VkDescriptorSetLayout layout)
    : device(device) {
  CreatePipelineLayout(layout);
  CreatePipeline(renderPass);
  model = CreateRef<Engine::Mesh>(device, "prefilterMesh");
  // model->CreateCube();
}

void PrefilterRenderingSystem::Render(VkCommandBuffer commandBuffer,
...
void PrefilterRenderingSystem::CreatePipeline(VkRenderPass renderPass) {
  ...
  pipeline = CreateUnique<Engine::Pipeline>(
      device, std::string(SHADERS_PATH) + "/prefilter_vert.spv",
      std::string(SHADERS_PATH) + "/prefilter_frag.spv", pipelineConfig);
}
...
void PrefilterRenderingSystem::CreatePipelineLayout(
    VkDescriptorSetLayout layout) {
