...
CubemapRenderingSystem::CubemapRenderingSystem(Engine::Device &device,
                                               VkRenderPass renderPass,
                                               VkDescriptorSetLayout layout)
    : device(device) {
  CreatePipelineLayout(layout);
  CreatePipeline(renderPass);
  model = CreateRef<Engine::Mesh>(device, "CubeMapMeshs");
  // model->CreateCube();
}

void CubemapRenderingSystem::Render(VkCommandBuffer commandBuffer,
...
void CubemapRenderingSystem::CreatePipeline(VkRenderPass renderPass) {
  ...
  pipeline = CreateUnique<Engine::Pipeline>(
      device, std::string(SHADERS_PATH) + "/show_cubemap_vert.spv",
      std::string(SHADERS_PATH) + "/show_cubemap_frag.spv", pipelineConfig);
}
...
void CubemapRenderingSystem::CreatePipelineLayout(
    VkDescriptorSetLayout layout) {
