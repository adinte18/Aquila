...
CompositeRenderingSystem::CompositeRenderingSystem(Engine::Device &device,
...
void CompositeRenderingSystem::Render(VkCommandBuffer commandBuffer,
...
void CompositeRenderingSystem::CreatePipeline(VkRenderPass renderPass) {
  ...
  pipeline = CreateUnique<Engine::Pipeline>(
      device, std::string(SHADERS_PATH) + "/final_vert.spv",
      std::string(SHADERS_PATH) + "/final_frag.spv", pipelineConfig);
}
