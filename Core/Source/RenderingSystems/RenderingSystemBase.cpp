...
void RenderingSystemBase::SetUniformData(
...
void RenderingSystemBase::SetTexture(uint32_t binding,
...
void RenderingSystemBase::AllocateDescriptorSet() {
  for (int i = 0; i < Swapchain::MAX_FRAMES_IN_FLIGHT; i++) {
    DescriptorAllocator::Allocate(m_Layout->GetDescriptorSetLayout(),
                                  m_DescriptorSets[i]);
  }
}
