...
EnvToCubemapRenderingSystem::EnvToCubemapRenderingSystem(
    Engine::Device &device, VkRenderPass renderPass,
    VkDescriptorSetLayout layout)
    : device(device) {
  CreatePipelineLayout(layout);
  CreatePipeline(renderPass);
  model = CreateRef<Engine::Mesh>(device, "envToCubemapMesh");
  // model->CreateCube();
}

void EnvToCubemapRenderingSystem::Render(VkCommandBuffer commandBuffer,
...
void EnvToCubemapRenderingSystem::CreatePipeline(VkRenderPass renderPass) {
  ...
  pipeline = CreateUnique<Engine::Pipeline>(
      device, std::string(SHADERS_PATH) + "/cubemap_vert.spv",
      std::string(SHADERS_PATH) + "/cubemap_frag.spv", pipelineConfig);
}
...
void EnvToCubemapRenderingSystem::CreatePipelineLayout(
    VkDescriptorSetLayout layout) {
