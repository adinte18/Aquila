...
PreethamSkyRenderingSystem::PreethamSkyRenderingSystem(
...
void PreethamSkyRenderingSystem::Render(VkCommandBuffer commandBuffer,
...
void PreethamSkyRenderingSystem::CreatePipeline(VkRenderPass renderPass) {
  ...
  pipeline = CreateUnique<Engine::Pipeline>(
      device, std::string(SHADERS_PATH) + "/atmosphere_vert.spv",
      std::string(SHADERS_PATH) + "/atmosphere_frag.spv", pipelineConfig);
}
...
void PreethamSkyRenderingSystem::CreatePipelineLayout(
    VkDescriptorSetLayout layout) {
