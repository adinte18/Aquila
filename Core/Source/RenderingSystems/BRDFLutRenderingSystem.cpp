...
void BRDFLutRenderingSystem::Render(VkCommandBuffer commandBuffer,
...
void BRDFLutRenderingSystem::CreatePipeline(VkRenderPass renderPass) {
  ...
  pipeline = CreateUnique<Engine::Pipeline>(
      device, std::string(SHADERS_PATH) + "/brdflut_vert.spv",
      std::string(SHADERS_PATH) + "/brdflut_frag.spv", pipelineConfig);
}

void BRDFLutRenderingSystem::CreatePipelineLayout(
    VkDescriptorSetLayout layout) {
