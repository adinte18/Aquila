...
DepthRenderingSystem::DepthRenderingSystem(
...
void DepthRenderingSystem::Render(VkCommandBuffer commandBuffer) const {
  ...
  // sceneContext.GetScene().GetRegistry().view<Engine::Light>().each([&sceneContext,
  // this, &commandBuffer](Engine::Light& light) {
  //     if (light.type == Engine::Light::LightType::Directional) {

  //         light.UpdateMatrices();

  //         sceneContext.GetScene().GetRegistry().view<Engine::Transform,
  //         Engine::MeshCmp>()
  //             .each([&sceneContext, this, &commandBuffer](Engine::Transform&
  //             transform, Engine::MeshCmp& mesh) {
  ...
  //                 // mesh.mesh->Draw(commandBuffer,
  //                 sceneContext.GetSceneDescriptorSet(), pipelineLayout);
  //             }
  //         });
  //     }
  // });
...
void DepthRenderingSystem::CreatePipeline(VkRenderPass renderPass) {
  ...
  pipeline = CreateUnique<Engine::Pipeline>(
      device, std::string(SHADERS_PATH) + "/shadows_vert.spv",
      std::string(SHADERS_PATH) + "/shadows_frag.spv", pipelineConfig);
}

void DepthRenderingSystem::CreatePipelineLayout(
    std::array<VkDescriptorSetLayout, 2> setLayouts) {
