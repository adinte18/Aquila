...
GridRenderingSystem::GridRenderingSystem(
...
void GridRenderingSystem::CreatePipeline(VkRenderPass renderPass) {
  ...
  pipeline = CreateUnique<Engine::Pipeline>(
      device, std::string(SHADERS_PATH) + "/grid_vert.spv",
      std::string(SHADERS_PATH) + "/grid_frag.spv", pipelineConfig);
}

void GridRenderingSystem::CreatePipelineLayout(
    std::array<VkDescriptorSetLayout, 2> setLayouts) {
