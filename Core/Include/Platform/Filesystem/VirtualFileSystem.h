...
public:
  bool Mount(const std::string &virtualPath,
             std::shared_ptr<IFileSystem> fileSystem, int priority = 0,
             bool readOnly = false);
  bool Unmount(const std::string &virtualPath);
  void UnmountAll();

  std::unique_ptr<VirtualFile> OpenFile(const std::string &virtualPath,
