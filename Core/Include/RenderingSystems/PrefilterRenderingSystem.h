...
public:
  PrefilterRenderingSystem(Device &device, VkRenderPass renderPass,
                           VkDescriptorSetLayout layout);
  ~PrefilterRenderingSystem() {
    vkDestroyPipelineLayout(device.GetDevice(), pipelineLayout, nullptr);
  };

  PrefilterRenderingSystem(const PrefilterRenderingSystem &) = delete;
  PrefilterRenderingSystem &
  operator=(const PrefilterRenderingSystem &) = delete;

  void Render(VkCommandBuffer commandBuffer, VkDescriptorSet descriptorSet,
              glm::mat4 &viewMatrix, uint32_t mipLevel);
