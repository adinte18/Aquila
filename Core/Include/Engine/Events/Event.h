...
enum class UIEventResult : uint16_t { Success, Failure };

using CommandParam = std::variant<int, float, bool, std::string, entt::entity,
...
struct UICommandEvent : Event {
  ...
  explicit UICommandEvent(
      const UICommand cmd,
      std::unordered_map<std::string_view, CommandParam> args = {},
