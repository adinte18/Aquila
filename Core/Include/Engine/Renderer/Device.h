...
private:
  ...
  QuerySwapChainSupport(VkPhysicalDevice vkPhysicalDevice);

  bool CheckValidationLayerSupport();

  static VkResult
  CreateDebugMessengerEXT(VkInstance instance,
  ...
  DebugCallback(VkDebugUtilsMessageSeverityFlagBitsEXT messageSeverity,
    ...
    std::cerr << severityColor << "[VULKAN " << severityText << "]"
              << resetColor;
    if (!typeStr.empty()) {
      std::cerr << " [" << typeStr << "]";
    }
    ...
    if (pCallbackData->pMessageIdName) {
      std::cerr << "  ID: " << pCallbackData->pMessageIdName << " ("
...
public:
  ...
  VkQueueFamilyIndices FindPhysicalQF() {
    return FindQueueFamilies(m_PhysicalDevice);
  }

  VkSwapChainSupportDetails GetSwapChainSupport() {
  ...
  void SetObjectDebugName(VkObjectType objectType, uint64_t handle,
                          const char *name);
