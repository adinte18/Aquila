...
void ContentElement::FolderActionsPopup() {
  if (ImGui::BeginPopup("FolderActions", ImGuiWindowFlags_NoMove)) {
    ...
    if (ImGui::MenuItem(ICON_LC_PEN " Rename")) {
      ...
      strncpy_s(m_RenameBuffer.data(), m_RenameBuffer.size(),
                currentName.c_str(), _TRUNCATE);
#elif defined(AQUILA_PLATFORM_LINUX)
      std::strncpy(m_RenameBuffer.data(), currentName.c_str(),
...
void ContentElement::RenameFolderPopup() {
  ...
  ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove)) {
      ...
      ImGui::InputText("##renameFolder", m_RenameBuffer.data(),
  m_RenameBuffer.size(), ImGuiInputTextFlags_EnterReturnsTrue);
  ...
  std::string(m_RenameBuffer.data());
...
void ContentElement::DeleteItemPopup() {
  ...
  if (ImGui::BeginPopupModal("Delete", nullptr,
    ...
    ImGui::TextWrapped("Item to be deleted: %s",
                       m_CurrentlyHoveredPath.c_str());
    ImGui::PopStyleColor();
    ImGui::PopFont();
    ...
    if (ImGui::Button(ICON_LC_CHECK " Yes", {90, 30})) {
      ...
      if (VFS::VirtualFileSystem::Get()->IsDirectory(m_CurrentlyHoveredPath)) {
        success = VFS::VirtualFileSystem::Get()->DeleteDirectory(
            m_CurrentlyHoveredPath);
      } else {
        success = VFS::VirtualFileSystem::Get()->DeleteFile_aq(
            m_CurrentlyHoveredPath);
      }
...
void ContentElement::Draw() {
  ...
  ImGui::BeginChild("Navigation", ImVec2(avail.x, 40), true,
  ...
  ImGui::BeginChild("Content",
                    ImVec2(content_width, avail.y - navigation_height), true);

  if (!s_CurrentPath.empty() &&
      VFS::VirtualFileSystem::Get()->Exists(s_CurrentPath) &&
      VFS::VirtualFileSystem::Get()->IsDirectory(s_CurrentPath)) {
    ...
    if (entries.empty()) {
      ...
      int cardsPerRow =
          std::max(1, static_cast<int>(content_width / (cardWidth + padding)));
      int cardIndex = 0;
      ...
      for (const auto &entryName : entries) {
        ...
        ImGui::BeginChild(
            "Card", ImVec2(cardWidth, cardHeight), true,
        ...
        if (isDirectory) {
          textureID =
              reinterpret_cast<ImTextureID>(m_Textures[0]->GetDescriptorSet());
        } else if (extension == ".aqscene") {
          textureID =
              reinterpret_cast<ImTextureID>(m_Textures[1]->GetDescriptorSet());
        } else {
          textureID =
              reinterpret_cast<ImTextureID>(m_Textures[2]->GetDescriptorSet());
        }

        if (textureID) {
          bool clicked =
              ImGui::ImageButton("##Icon", textureID,
                                 ImVec2(cardWidth - 3.5f * iconPadding,
                                        cardWidth - 2 * iconPadding));
          ...
          if (extension == ".aqscene") {
            if (ImGui::BeginDragDropSource(
              ...
              ImGui::SetDragDropPayload("SCENE_PATH", fullPath.c_str(),
                                        fullPath.size() + 1);
          ...
          if (extension == ".gltf" || extension == ".glb" ||
            ...
            if (ImGui::BeginDragDropSource(
                    ImGuiDragDropFlags_SourceAllowNullID)) {
              ImGui::SetDragDropPayload("MESH_ASSET", fullPath.c_str(),
                                        fullPath.size() + 1);
    ...
    ImGui::TextColored(ImVec4(1, 0, 0, 1),
                       "Invalid directory or VFS mount point not found");
  }
